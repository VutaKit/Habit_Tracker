...
def _getDb():
    return _getMondoClient()[dbName]


# Data extraction from mongoDB
def extractData(collection, pipeline=[]):
    db = _getDb()
    docs = list(db[collection].aggregate(pipeline))
    print(f"Got {len(docs)} docs from {collection}")
    # print(docs)
    return docs


# Importing data
def importDataHabits():
    global habit_list

    # Импорт пронумерованных узлов из БД
    habit_list = extractData(
        collection="habit_list",
    )


# Export to MongoDB function
def exportToMongo(data, collectionName):
    collection = _getDb()[collectionName]
    collection.drop()
